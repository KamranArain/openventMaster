...
#define decisionTimeStatusDeclaration 3 //10 * 10 = 100 ms
  ...
  if (HealthCtr >= decisionTime)
    status.sensorsHealth = HEALTH_GOOD;
  else if (HealthCtr <= 0)
    status.sensorsHealth = HEALTH_BAD;
  ...
  if (FS_HealthCtr >= decisionTimeStatusDeclaration)
    status.flowSensorFailure = false;
  else if (FS_HealthCtr <= (-1 * decisionTimeStatusDeclaration))
  ...
  if (PS_HealthCtr >= decisionTimeStatusDeclaration)
    status.presSensorFailure = false;
  else if (PS_HealthCtr <= (-1 * decisionTimeStatusDeclaration))
